...
import { initialViewState } from "@safari/util/constants";
import { SearchPlaceBox } from "../organisms/map/SearchPlacesBox";
import { useFormContext } from "react-hook-form";
import { FormTypeSearchGarage } from "@safari/forms/src/searchGarages";
import { IconType } from "../molecules/IconTypes";
import { IconArrowDown } from "@tabler/icons-react";
import { HtmlInput } from "../atoms/HtmlInput";
import { toLocalISOString } from "@safari/util/date";
...
export const SearchPage = () => {
  ...
  const handleMapChange = useCallback(
    (target: ViewStateChangeEvent["target"]) => {
      if (!target || typeof target.getBounds !== "function") {
        console.warn("Map target is not ready yet.");
        return;
      }
  ...
  return (
    <Map
      onLoad={(e) => setTimeout(() => handleMapChange(e.target), 500)}
